"""Persistencia SQLite — organizaciones (dominio) y usuarios (Google + rol)."""
...
class AuthDB:
    ...
    def get_org_by_tenant(self, tenant_key: str) -> Optional[OrgRow]:
        with self._lock:
            ...
            try:
                cur = conn.execute(
                    "SELECT id, tenant_key, created_at FROM organizations WHERE tenant_key = ?",
                    (tenant_key.lower().strip(),),
                )
                row = cur.fetchone()
                if row is None:
                    return None
                return OrgRow(id=row["id"], tenant_key=row["tenant_key"], created_at=row["created_at"])
            finally:
                conn.close()

    def create_org(self, tenant_key: str) -> OrgRow:
        tk = tenant_key.lower().strip()
        now = _utc_now_iso()
        with self._lock:
            conn = self._connect()
            try:
                ...
                return OrgRow(id=oid, tenant_key=tk, created_at=now)
            finally:
                conn.close()
